...
import { Client, FacultyRecord, FacultyMember, MatcherSearchFilters, MentorEvaluationSnapshot, MentorRecommendation, RecommendationOrigin, SourceMode } from './types';
...
function App() {
  ...
  const addFacultyToDatabase = (faculty: FacultyMember, manualCountry?: string, manualField?: string, extra?: Partial<FacultyRecord>): string => {
    ...
    let recordId = incomingRecord.id;
    setFacultyDatabase(prev => {
      const result = upsertFacultyRecord(prev, incomingRecord);
      recordId = result.id;
    ...
    const updatedClients = clients.map(c => {
      if (c.id === clientId) {
        ...
        if (selectedClient?.id === clientId) {
          upsertSelectedClient(updatedClient);
        }
  ...
  const openClientDetail = (client: Client, initialTab: 'profile' | 'documents' | 'mentors' = 'profile') => {
    setPreviousTab(activeTab);
    upsertSelectedClient(client);
    setClientDetailInitialTab(initialTab);
    setActiveTab('users');
  };

  const openFacultyDbForClient = (client: Client) => {
    upsertSelectedClient(client);
    setClientDetailInitialTab('mentors');
    setFacultyDbContext({
      clientId: client.id,
      filters: buildMatcherFiltersFromClient(client),
    });
    setActiveTab('faculty-db');
  };
      ...
      <div className="h-full w-full relative">
        ...
        {activeTab === 'users' && selectedClient && (
          <div className="absolute inset-0 z-20 bg-[#F7F8FA]">
            <ClientDetail 
              ...
              onBack={() => {
                upsertSelectedClient(null);
                setClientDetailInitialTab('profile');
                setActiveTab('users');
              }} 
              initialTab={clientDetailInitialTab}
              onOpenAdvancedFacultyFilters={openFacultyDbForClient}
              onStartWriting={(type) => {
                ...
                if (type) {
                  navigateToWorkbench(tabMap[type] || 'freewrite');
                } else {
                  navigateToWorkbench('freewrite');
                }
              }}
              onEditDocument={(doc) => {
                ...
                navigateToWorkbench(typeMap[doc.type] || 'freewrite');
              }}
        ...
        <div className={activeTab === 'dashboard' ? 'block h-full' : 'hidden'}>
          <Dashboard 
            ...
            clients={clients} 
            onSelectClient={(client) => openClientDetail(client, 'profile')} 
            onUpdateClient={updateClient}
          />
        </div>
        ...
        <div className={activeTab === 'faculty-db' ? 'block h-full' : 'hidden'}>
          <FacultyDatabase 
              ...
              contextClient={facultyDbContext ? clients.find((client) => client.id === facultyDbContext.clientId) || null : null}
              contextFilters={facultyDbContext?.filters}
              onReturnToClient={() => {
                if (facultyDbContext?.clientId) {
                  const client = clients.find((item) => item.id === facultyDbContext.clientId);
                  if (client) {
                    openClientDetail(client, 'mentors');
                  }
                }
                setFacultyDbContext(null);
              }}
        ...
        <div className={activeTab === 'projects' ? 'block h-full' : 'hidden'}>
          <MyWorks 
            clients={clients} 
            onCreateNew={() => navigateToWorkbench('freewrite')} 
            onEditDocument={(doc) => {
              const typeMap: Record<string, TabId> = {
                'PS': 'ps',
                'Essay': 'essay',
        ...
        <div className={activeTab === 'agent' ? 'block h-full' : 'hidden'}>
          <EssayAgentEntry 
            clients={clients} 
            onAddClient={(name, parsedData) => addClient({ name, ...parsedData })} 
            onSelectClient={(client) => openClientDetail(client, 'documents')} 
          />
        </div>

        <div className={activeTab === 'ps' ? 'block h-full' : 'hidden'}>
          <PSWorkbench clients={clients} onAddClientClick={() => setIsCreateClientModalOpen(true)} onSaveDocument={saveDocument} initialDocument={editingDocument?.type === 'PS' ? editingDocument : undefined} onBack={handleWorkbenchBack} initialClientId={selectedClient?.id} />
        </div>

        <div className={activeTab === 'essay' ? 'block h-full' : 'hidden'}>
          <PromptEssayWorkbench clients={clients} onAddClient={() => setIsCreateClientModalOpen(true)} onSaveDocument={saveDocument} initialDocument={editingDocument?.type === 'Essay' ? editingDocument : undefined} onBack={handleWorkbenchBack} initialClientId={selectedClient?.id} />
        </div>

        <div className={activeTab === 'lor' ? 'block h-full' : 'hidden'}>
          <LORWorkbench clients={clients} onAddClient={() => setIsCreateClientModalOpen(true)} onSaveDocument={saveDocument} initialDocument={editingDocument?.type === 'LOR' ? editingDocument : undefined} onBack={handleWorkbenchBack} initialClientId={selectedClient?.id} />
        </div>

        <div className={activeTab === 'cv' ? 'block h-full' : 'hidden'}>
          <CVWorkbench clients={clients} onAddClient={() => setIsCreateClientModalOpen(true)} onSaveDocument={saveDocument} initialDocument={editingDocument?.type === 'CV' ? editingDocument : undefined} onBack={handleWorkbenchBack} initialClientId={selectedClient?.id} />
        </div>

        <div className={activeTab === 'freewrite' ? 'block h-full' : 'hidden'}>
          <FreeWriteWorkbench clients={clients} onTabChange={handleTabChange} onAddClientClick={() => setIsCreateClientModalOpen(true)} onSaveDocument={saveDocument} initialDocument={editingDocument?.type === 'Free Writing' ? editingDocument : undefined} onBack={handleWorkbenchBack} initialClientId={selectedClient?.id} />
        </div>
