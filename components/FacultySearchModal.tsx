...
const FacultySearchModal: React.FC<FacultySearchModalProps> = ({ isOpen, onClose, onImport }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative glass w-full max-w-4xl h-[80vh] rounded-3xl shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200 border border-white/50">
          ...
          {isSearching ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {results.map((faculty, index) => (
                <div 
                  key={index} 
                  className={`relative group cursor-pointer transition-all ${selectedIndices.includes(index) ? 'ring-2 ring-blue-500 rounded-[40px] shadow-lg shadow-blue-500/10' : ''}`}
                ...
                >
                  ...
                  <div className={`absolute top-4 right-4 w-6 h-6 rounded-full border-2 flex items-center justify-center transition-colors shadow-sm ${
                    selectedIndices.includes(index) ? 'bg-blue-600 border-blue-600 text-white' : 'bg-white/80 backdrop-blur-sm border-gray-200 text-transparent group-hover:border-blue-300'
                  }`}>
        ...
        {/* Footer */}
        <div className="px-6 py-4 bg-white/40 backdrop-blur-sm border-t border-white/50 flex justify-between items-center">
          ...
          <div className="flex space-x-3">
            ...
            <button 
              ...
              className="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl text-sm font-bold hover:from-blue-700 hover:to-indigo-700 transition-all shadow-md shadow-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center active:scale-95"
            >
