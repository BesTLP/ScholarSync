...
const CVSidebarConfig: React.FC<CVSidebarConfigProps> = ({ 
  ...
  return (
    <div className={`${isCollapsed ? 'w-[72px]' : 'w-[300px]'} relative bg-white border-r border-gray-100 flex flex-col h-full overflow-visible custom-scrollbar transition-all duration-300`}>
      <div className={`border-b border-gray-100 p-4 flex items-center ${isCollapsed ? 'justify-center' : 'justify-start'}`}>
        {!isCollapsed && <h3 className="text-base font-bold text-gray-900">简历</h3>}
      </div>
      {isCollapsed ? (
